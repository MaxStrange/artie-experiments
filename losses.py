...
def reconstruction_loss(pred_batch, label_batch):
    ...
    batch_sums_of_num_zeros_in_label = torch.sum(mask_zeros_in_label, (1, 2), dtype=label_batch.dtype)
    mask_nonzeros_in_label = (label_batch > 0.0).int()
    batch_sums_of_num_non_zeros_in_label = torch.sum(mask_nonzeros_in_label, (1, 2), dtype=label_batch.dtype)
    ...
    difference = nn.functional.l1_loss(pred_batch, label_batch)

    # diff[mask_nonzeros_in_label] should get multiplied by the appropriate balanceing factor
    balanced_difference = difference * balances
    return torch.mean(balanced_difference)
...
if __name__ == "__main__":
    pred_batch = torch.rand((2, 10, 5), dtype=torch.float32, requires_grad=True)
